...
export default function DashboardStats({ guests }: { guests: Guest[] }) {
    ...
    const totalConfirmedPeople = confirmed.reduce((acc, g) => acc + 1 + g.confirmedCompanions.length, 0);
    const childrenConfirmations = guests.reduce((acc, g) => acc + (g.confirmed ? g.childrenCount : 0), 0);
    ...
    const stats = [
        ...
        { label: 'Total de Pessoas', value: totalConfirmedPeople, color: 'text-gold-dark', bg: 'bg-primary-50' },
    ];
